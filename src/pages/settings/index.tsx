...
import { getSettings, deleteSettingsById } from 'apiSdk/settings';
import { SettingsInterface } from 'interfaces/settings';

type ColumnType = ColumnDef<SettingsInterface, unknown>;
...
export function SettingsListPage(props: SettingsListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getSettings({
        ...
        order: params.order,
        searchTermKeys: ['currency.contains', 'timezone.contains', 'language.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<SettingsInterface>>(
  ...
  const handleView = (row: SettingsInterface) => {
    if (hasAccess('settings', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/settings/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'invoice_due_days', header: 'invoice_due_days', accessorKey: 'invoice_due_days' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/settings/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('settings', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/settings/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('settings', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="settings" />
        </Flex>

        {hasAccess('settings', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/settings/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
